...
void build(vi &arr, vi &tree, ll s, ll e, ll ti){
    ...
    tree[ti] = max(tree[2*ti], tree[2*ti+1]);
}
...
int query(vi &tree, ll s, ll e, ll ti, ll x){
    if(tree[ti]<x) return  INT_MAX;
    if(s==e ) {
       return tree[ti];
    }
    int m = mid(s,e);
    int a,b;
    if(tree[2*ti]>=x){
        a = query(tree, s, m, 2*ti, x);
    }  else {
        a =INT_MAX;
    }
    if(tree[2*ti+1]>=x){
        b =query(tree, m+1, e, 2+ti+1, x);
    } else {
        b = INT_MAX;
    }
    return min(a,b);
...
int main(int argc, char const *argv[]) {
    ...
    while(q--){
        ...
        if(type==1){
            ...
            cin>>x;
            cout<<query(tree, 0, n-1, 1, x)<<endl;
        }
    }
	#ifndef ONLINE_JUDGE 
	  clock_t end = clock();
